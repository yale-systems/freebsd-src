...
{
    sx_slock(&disk_head_lock);
}

void
vtab_disk_list_unlock(void)
{
    sx_sunlock(&disk_head_lock);
}

void
vtab_disk_list_snapshot(sqlite3_vtab *pVtab, struct timespec when)
{
    struct disk_list *prc = LIST_FIRST(&disk_head);

    osdb_snap *snap = malloc(sizeof(struct osdb_snap), M_SQLITE, M_WAITOK);
    snap->when = when;
    snap->snap_table = new_osdb_table(VT_disk_head_NUM_COLUMNS);
    MD5Init(&snap->context);

    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_disk_head_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
static int
vtab_disk_list_rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    common_cursor *pCur = (common_cursor *)cur;
    osdb_value *pid_value = pCur->row->columns[VT_disk_head_PID];
    *pRowid = pid_value->int64_value;
    printf("disk_list_rowid was called, returning %lld\n", *pRowid);
    return SQLITE_OK;
}

static int
vtab_disk_list_bestindex(sqlite3_vtab *tab, sqlite3_index_info *pIdxInfo)
{
...
static int
vtab_disk_list_update(sqlite3_vtab *pVTab, int argc, sqlite3_value **argv, sqlite_int64 *pRowid)
{
    struct timespec when;
    nanotime(&when);
    vtab_disk_list_snapshot(pVTab, when);
    if (osdb_snapshot_compare((struct osdb_vtab *)pVTab) <= 0) {
#ifdef DEBUG
        printf("disk_list digest mismatch: UPDATE failed\n");
#endif
        return SQLITE_ABORT;
    }
...
** virtual table.
*/
static sqlite3_module disk_listvtabModule = {
    /* iVersion    */ 0,
    /* xCreate     */ commonCreate,
    /* xConnect    */ commonConnect,
    /* xBestIndex  */ disk_listvtabBestIndex,
    ...
    /* xRowid      */ disk_listvtabRowid,
    /* xUpdate     */ disk_listvtabUpdate,
    /* xBegin      */ 0,
    /* xSync       */ 0,
    /* xCommit     */ 0,
    /* xRollback   */ 0,
...
int
sqlite3_disk_listvtab_init(sqlite3 *db, char **pzErrMsg,
    const sqlite3_api_routines *pApi, void *pAux)
{
    SQLITE_EXTENSION_INIT2(pApi);
    return sqlite3_create_module(db,
        vtable_type_to_name(((osdb_vtab *)pAux)->type), &disk_listvtabModule,
        pAux);
}
