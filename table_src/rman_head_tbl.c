...
void
vtab_rman_head_unlock(void)
{
    sx_sunlock(&rman_head_lock);
}

void
vtab_rman_head_snapshot(sqlite3_vtab *pVtab, struct timespec when)
{
    struct rman_head *prc = LIST_FIRST(&rman_head);
    ...
    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_rman_head_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
static int
vtab_rman_head_rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    common_cursor *pCur = (common_cursor *)cur;
    osdb_value *pid_value = pCur->row->columns[VT_rman_head_PID];
    *pRowid = pid_value->int64_value;
    printf("rman_head_rowid was called, returning %lld\n", *pRowid);
    return SQLITE_OK;
}

static int
vtab_rman_head_bestindex(sqlite3_vtab *tab, sqlite3_index_info *pIdxInfo)
{
...
static int
vtab_rman_head_update(sqlite3_vtab *pVTab, int argc, sqlite3_value **argv, sqlite_int64 *pRowid)
{
    struct timespec when;
    nanotime(&when);
    vtab_rman_head_snapshot(pVTab, when);
    if (osdb_snapshot_compare((struct osdb_vtab *)pVTab) <= 0) {
#ifdef DEBUG
        printf("rman_head digest mismatch: UPDATE failed\n");
#endif
        return SQLITE_ABORT;
    }
...
** virtual table.
*/
static sqlite3_module rman_headvtabModule = {
    /* iVersion    */ 0,
    /* xCreate     */ commonCreate,
    /* xConnect    */ commonConnect,
    /* xBestIndex  */ rman_headvtabBestIndex,
    ...
    /* xRowid      */ rman_headvtabRowid,
    /* xUpdate     */ rman_headvtabUpdate,
    /* xBegin      */ 0,
    /* xSync       */ 0,
    /* xCommit     */ 0,
    /* xRollback   */ 0,
...
int
sqlite3_rman_headvtab_init(sqlite3 *db, char **pzErrMsg,
    const sqlite3_api_routines *pApi, void *pAux)
{
    SQLITE_EXTENSION_INIT2(pApi);
    return sqlite3_create_module(db,
        vtable_type_to_name(((osdb_vtab *)pAux)->type), &rman_headvtabModule,
        pAux);
}
