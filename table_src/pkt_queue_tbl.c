...
{
    sx_slock(&pkt_queue_lock);
}

void
vtab_pkthead_unlock(void)
{
    sx_sunlock(&pkt_queue_lock);
}

void
vtab_pkthead_snapshot(sqlite3_vtab *pVtab, struct timespec when)
{
    struct pkthead *prc = LIST_FIRST(&pkt_queue);

    osdb_snap *snap = malloc(sizeof(struct osdb_snap), M_SQLITE, M_WAITOK);
    snap->when = when;
    snap->snap_table = new_osdb_table(VT_pkt_queue_NUM_COLUMNS);
    MD5Init(&snap->context);

    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_pkt_queue_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
vtab_pkthead_rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    ...
    osdb_value *pid_value = pCur->row->columns[VT_pkt_queue_PID];
