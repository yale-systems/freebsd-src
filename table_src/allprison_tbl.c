...
void
vtab_prisonlist_unlock(void)
{
    sx_sunlock(&allprison_lock);
}

void
vtab_prisonlist_snapshot(sqlite3_vtab *pVtab, struct timespec when)
{
    struct prisonlist *prc = LIST_FIRST(&allprison);
    ...
    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_allprison_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
static int
vtab_prisonlist_rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    common_cursor *pCur = (common_cursor *)cur;
    osdb_value *pid_value = pCur->row->columns[VT_allprison_PID];
    *pRowid = pid_value->int64_value;
    printf("prisonlist_rowid was called, returning %lld\n", *pRowid);
    return SQLITE_OK;
}

static int
vtab_prisonlist_bestindex(sqlite3_vtab *tab, sqlite3_index_info *pIdxInfo)
{
...
static int
vtab_prisonlist_update(sqlite3_vtab *pVTab, int argc, sqlite3_value **argv, sqlite_int64 *pRowid)
{
    struct timespec when;
    nanotime(&when);
    vtab_prisonlist_snapshot(pVTab, when);
    if (osdb_snapshot_compare((struct osdb_vtab *)pVTab) <= 0) {
#ifdef DEBUG
        printf("prisonlist digest mismatch: UPDATE failed\n");
#endif
        return SQLITE_ABORT;
    }
...
** virtual table.
*/
static sqlite3_module prisonlistvtabModule = {
    /* iVersion    */ 0,
    /* xCreate     */ commonCreate,
    /* xConnect    */ commonConnect,
    /* xBestIndex  */ prisonlistvtabBestIndex,
    ...
    /* xRowid      */ prisonlistvtabRowid,
    /* xUpdate     */ prisonlistvtabUpdate,
    /* xBegin      */ 0,
    /* xSync       */ 0,
    /* xCommit     */ 0,
    /* xRollback   */ 0,
...
int
sqlite3_prisonlistvtab_init(sqlite3 *db, char **pzErrMsg,
    const sqlite3_api_routines *pApi, void *pAux)
{
    SQLITE_EXTENSION_INIT2(pApi);
    return sqlite3_create_module(db,
        vtable_type_to_name(((osdb_vtab *)pAux)->type), &prisonlistvtabModule,
        pAux);
}
