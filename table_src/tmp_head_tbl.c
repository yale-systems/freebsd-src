...
static int
copy_columns(struct tmp_head *curEntry, osdb_value **columns, struct timespec *when, MD5_CTX *context) {

//    columns[VT_tmp_head_] =  TODO: Handle other types
//    columns[VT_tmp_head_func] =  TODO: Handle other types
...
{
    struct  *prc = LIST_FIRST(&tmp_head);
    ...
    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_tmp_head_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
vtab__rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    ...
    osdb_value *pid_value = pCur->row->columns[VT_tmp_head_PID];
