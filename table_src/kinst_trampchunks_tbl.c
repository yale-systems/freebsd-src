...
{
    struct  *prc = LIST_FIRST(&kinst_trampchunks);
    ...
    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_kinst_trampchunks_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
vtab__rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    ...
    osdb_value *pid_value = pCur->row->columns[VT_kinst_trampchunks_PID];
