...
static int
copy_columns(struct iql *curEntry, osdb_value **columns, struct timespec *when, MD5_CTX *context) {
...
vtab__snapshot(sqlite3_vtab *pVtab, struct timespec when)
{
    ...
    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_iql_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
vtab__rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    ...
    osdb_value *pid_value = pCur->row->columns[VT_iql_PID];
