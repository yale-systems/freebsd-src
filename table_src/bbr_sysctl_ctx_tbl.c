...
enum col {
    VT_bbr_sysctl_ctx_entry = 0,
    VT_bbr_sysctl_ctx_link = 1,
    VT_bbr_sysctl_ctx_NUM_COLUMNS
};

static int
copy_columns(struct bbr_sysctl_ctx *curEntry, osdb_value **columns, struct timespec *when, MD5_CTX *context) {

//    columns[VT_bbr_sysctl_ctx_entry] =  TODO: Handle other types
//    columns[VT_bbr_sysctl_ctx_link] =  TODO: Handle other types
...
{
    sx_slock(&bbr_sysctl_ctx_lock);
}

void
vtab_sysctl_ctx_list_unlock(void)
{
    sx_sunlock(&bbr_sysctl_ctx_lock);
}

void
vtab_sysctl_ctx_list_snapshot(sqlite3_vtab *pVtab, struct timespec when)
{
    struct sysctl_ctx_list *prc = LIST_FIRST(&bbr_sysctl_ctx);

    osdb_snap *snap = malloc(sizeof(struct osdb_snap), M_SQLITE, M_WAITOK);
    snap->when = when;
    snap->snap_table = new_osdb_table(VT_bbr_sysctl_ctx_NUM_COLUMNS);
    MD5Init(&snap->context);

    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_bbr_sysctl_ctx_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
vtab_sysctl_ctx_list_rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    ...
    osdb_value *pid_value = pCur->row->columns[VT_bbr_sysctl_ctx_PID];
