...
static int
copy_columns(struct firmware_table *curEntry, osdb_value **columns, struct timespec *when, MD5_CTX *context) {

    columns[VT_firmware_table_refcnt] = new_osdb_int64(curEntry->refcnt, context);
//    columns[VT_firmware_table_link] =  TODO: Handle other types
//    columns[VT_firmware_table_parent] =  TODO: Handle other types
    columns[VT_firmware_table_flags] = new_osdb_int64(curEntry->flags, context);
//    columns[VT_firmware_table_file] =  TODO: Handle other types
//    columns[VT_firmware_table_fw] =  TODO: Handle other types
...
{
    struct  *prc = LIST_FIRST(&firmware_table);
    ...
    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_firmware_table_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
vtab__rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    ...
    osdb_value *pid_value = pCur->row->columns[VT_firmware_table_PID];
