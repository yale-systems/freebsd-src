...
static int
copy_columns(struct twr_list *curEntry, osdb_value **columns, struct timespec *when, MD5_CTX *context) {
...
{
    struct  *prc = LIST_FIRST(&twr_list);
    ...
    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_twr_list_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
vtab__rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    ...
    osdb_value *pid_value = pCur->row->columns[VT_twr_list_PID];
