...
vtab__lock(void)
{
    sx_slock(&linux_vma_head_lock);
}

void
vtab__unlock(void)
{
    sx_sunlock(&linux_vma_head_lock);
}

void
vtab__snapshot(sqlite3_vtab *pVtab, struct timespec when)
{
    struct  *prc = LIST_FIRST(&linux_vma_head);

    osdb_snap *snap = malloc(sizeof(struct osdb_snap), M_SQLITE, M_WAITOK);
    snap->when = when;
    snap->snap_table = new_osdb_table(VT_linux_vma_head_NUM_COLUMNS);
    MD5Init(&snap->context);

    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_linux_vma_head_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
vtab__rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    ...
    osdb_value *pid_value = pCur->row->columns[VT_linux_vma_head_PID];
