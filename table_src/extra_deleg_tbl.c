...
{
    sx_slock(&extra_deleg_lock);
}

void
vtab_nfscldeleghead_unlock(void)
{
    sx_sunlock(&extra_deleg_lock);
}

void
vtab_nfscldeleghead_snapshot(sqlite3_vtab *pVtab, struct timespec when)
{
    struct nfscldeleghead *prc = LIST_FIRST(&extra_deleg);

    osdb_snap *snap = malloc(sizeof(struct osdb_snap), M_SQLITE, M_WAITOK);
    snap->when = when;
    snap->snap_table = new_osdb_table(VT_extra_deleg_NUM_COLUMNS);
    MD5Init(&snap->context);

    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_extra_deleg_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
vtab_nfscldeleghead_rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    ...
    osdb_value *pid_value = pCur->row->columns[VT_extra_deleg_PID];
