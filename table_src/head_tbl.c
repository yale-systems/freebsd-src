...
static int
copy_columns(struct head *curEntry, osdb_value **columns, struct timespec *when, MD5_CTX *context) {

//    columns[VT_head_vm] =  TODO: Handle other types
//    columns[VT_head_cdev] =  TODO: Handle other types
//    columns[VT_head_ucred] =  TODO: Handle other types
//    columns[VT_head_link] =  TODO: Handle other types
//    columns[VT_head_devmem] =  TODO: Handle other types
    columns[VT_head_flags] = new_osdb_int64(curEntry->flags, context);
...
vtab__snapshot(sqlite3_vtab *pVtab, struct timespec when)
{
    ...
    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_head_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
vtab__rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    ...
    osdb_value *pid_value = pCur->row->columns[VT_head_PID];
