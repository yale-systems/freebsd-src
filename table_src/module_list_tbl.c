...
{
    struct  *prc = LIST_FIRST(&module_list);
    ...
    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_module_list_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
vtab__rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    ...
    osdb_value *pid_value = pCur->row->columns[VT_module_list_PID];
