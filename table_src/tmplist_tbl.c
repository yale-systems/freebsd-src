...
static int
copy_columns(struct tmplist *curEntry, osdb_value **columns, struct timespec *when, MD5_CTX *context) {

//    columns[VT_tmplist_pt_next] =  TODO: Handle other types
//    columns[VT_tmplist_pt_td] =  TODO: Handle other types
//    columns[VT_tmplist_pt_pmcs] =  TODO: Handle other types
...
vtab__snapshot(sqlite3_vtab *pVtab, struct timespec when)
{
    ...
    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_tmplist_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
vtab__rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    ...
    osdb_value *pid_value = pCur->row->columns[VT_tmplist_PID];
