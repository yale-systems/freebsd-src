...
static int
copy_columns(struct vg_list *curEntry, osdb_value **columns, struct timespec *when, MD5_CTX *context) {

//    columns[VT_vg_list_vg_next] =  TODO: Handle other types
//    columns[VT_vg_list_vg_name] =  TODO: Handle other types
//    columns[VT_vg_list_vg_uuid] =  TODO: Handle other types
    columns[VT_vg_list_vg_extentsize] = new_osdb_int64(curEntry->vg_extentsize, context);
    columns[VT_vg_list_vg_sectorsize] = new_osdb_int64(curEntry->vg_sectorsize, context);
//    columns[VT_vg_list_vg_geom] =  TODO: Handle other types
//    columns[VT_vg_list_vg_pvs] =  TODO: Handle other types
//    columns[VT_vg_list_vg_lvs] =  TODO: Handle other types
...
vtab__snapshot(sqlite3_vtab *pVtab, struct timespec when)
{
    ...
    while (prc) {
        osdb_value **columns = new_osdb_columns(VT_vg_list_NUM_COLUMNS);
        if (!columns) {
            return;
        }
...
vtab__rowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid)
{
    ...
    osdb_value *pid_value = pCur->row->columns[VT_vg_list_PID];
